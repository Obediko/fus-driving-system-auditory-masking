...
**Authors of this adaptation:** Apochi Obed and Nikolai Axmacher, Ruhr
...
| Parameter | Setting |
| --- | --- |
| Generator | Image Guided Therapy D054 |
| Driving-system profile | `IGT-32-ch_comb_1x10-ch_DORTMUND` |
| Transducer | Imasonic `IS_PCD15287_01005`, 10 elements |
| Ultrasound carrier frequency | 300 kHz |
| Pulse repetition frequency | 5 Hz |
| Pulse duration | 20 ms |
| Duty cycle | 10% |
| Stimulation duration | 90 seconds |
| Number of pulses | 450 |
| Auditory pre-mask | 1 second |
| Auditory post-mask | 1 second |
| Total masking duration | 92 seconds |
| Default session condition | Sham |

The headphone output contains an audible carrier and background noise. It does
not reproduce the 300 kHz ultrasound carrier. The audible envelope follows the
5 Hz stimulation pattern, and a fixed random seed permits the same masking
waveform to be reproduced in active and sham sessions.

## What active and sham actually do

An **active** session connects to the IGT generator, loads the configured
sequence, starts the headphone mask, waits for the pre-mask interval, and calls
the original IGT `execute_sequence()` method. If the generator, transducer,
conversion files, and connection are correctly configured, this command
energizes the transducer and delivers the programmed ultrasound sequence.

A **sham** session plays the same masking waveform for the same duration, but
does not connect to the IGT generator or call `execute_sequence()`. The
transducer is therefore not energized through this software pathway.

The graphical session selector is visible to the operator. Consequently, the
interface alone does not establish double blinding. A genuinely double-blind
study requires an independently concealed randomization and allocation
procedure, together with checks for audible, visual, and equipment-related
differences between conditions.

## Windows installation

Use the existing Python virtual environment supplied with the FUS system.
Running these commands with a system-wide Python installation, particularly
Python 3.14, can cause NumPy or SciPy installation failures.

```powershell
cd C:\Users\Neuro\FUS\Radboud-FUS-driving-system-software
& C:\Users\Neuro\Envs\FUS_DS_PACKAGE\Scripts\Activate.ps1

python -c "import sys, numpy, scipy; print(sys.executable); print(numpy.__version__); print(scipy.__version__)"
python -m pip install sounddevice
python -m pip install -e .\fus_ds_package
```

After activation, the terminal prompt should include `(FUS_DS_PACKAGE)`. If
NumPy and SciPy already import successfully, do not reinstall the complete
requirements file just to add auditory masking.

## Launch and run

Start the masking interface from the activated environment:

```powershell
python .\standalone_driving_system_software\auditory_masking_gui.py
```

The interface supports headphone selection, waveform preview, matched-pulse
and background-noise settings, frozen WAV export, headphone-calibration
confirmation, and active or sham execution. Selecting **Active** does not
trigger stimulation by itself. Ultrasound starts only after the session is
started, the explicit warning is confirmed, all checks pass, and the existing
IGT driver successfully executes the programmed sequence.

The same protocol can be opened in Spyder:

```text
standalone_driving_system_software/standalone_igt_pmErC.py
```

The default condition is sham. An active command-line session can be requested
explicitly:
...
Before any participant session, verify the generator connection, transducer
identity, conversion files, acoustic coupling, targeting, pressure limits,
emergency stop, and all institutional and ethical approvals. Test and calibrate
the headphones with an actual sound-pressure measurement. The software blocks
execution until headphone calibration has been acknowledged.

The audio onset is software-timed and includes an estimate of device latency.
It is not equivalent to a shared hardware trigger. Claims about exact
audio-ultrasound synchronization require independent measurement of both
signals. Hardware-free tests establish software behavior, not measured
acoustic output or safety in humans.
...
- `CITATION.cff`: machine-readable citation naming Apochi Obed and Nikolai
  Axmacher as authors of this integrated software adaptation.
...
- `THIRD_PARTY_LICENSES.md`: separation of upstream MIT licensing from
  research-only masking components.
- `standalone_driving_system_software/auditory_masking_gui.py`: graphical
  masking and session-control application.
- `standalone_driving_system_software/standalone_igt_pmErC.py`: integrated
  Dortmund IGT active/sham protocol.
- `standalone_driving_system_software/masking_config_pmErC.json`: default
  waveform, timing, audio, and calibration settings.
- `fus_ds_package/fus_driving_systems/auditory_masking/`: masking engine,
  session orchestration, playback, and vendor attribution.
- `fus_ds_package/fus_driving_systems/tests/test_auditory_masking.py`:
  hardware-free regression tests.

The original Radboud FUS Driving System remains under the MIT terms in
[`LICENSE`](LICENSE). Code derived from the auditory masking work by Hira
Musarrat and Benjamin Kop is governed separately by the research-only license
and EULA in `fus_ds_package/fus_driving_systems/auditory_masking/_vendor/`.
...
The upstream project's original documentation is retained below.
...
**Dortmund/IfADo auditory masking:** This customized copy includes a research-only
auditory-masking adapter for the IGT D054 and Imasonic `IS_PCD15287_01005`.
The 300 kHz pmErC protocol is synchronized to a deterministic 5 Hz, 20 ms,
90-second masking track shared by active and sham sessions. Setup, headphone
calibration, safety limitations, the graphical interface, and third-party
license requirements are documented in
[AUDITORY_MASKING.md](AUDITORY_MASKING.md). The original Radboud driver remains
MIT-licensed; the bundled third-party masking code has separate non-commercial
research-only terms described in
