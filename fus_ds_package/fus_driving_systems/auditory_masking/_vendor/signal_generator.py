# Adapted from Hira Musarrat and Benjamin Kop, Auditory Mask Generator.
# Copyright (c) 2026 Benjamin Kop and Hira Musarrat.
# Research use only. See LICENSE, EULA.md, and NOTICE.md in this directory.
...
from .utils import (
    linear_ramp_window,
    tukey_ramp_window,
    exponential_ramp_window,
    logarithmic_ramp_window,
)
...
def generate_colored_noise(duration, sr, color_name):
    ...
    if beta == 0.0:
        return np.asarray(np.random.standard_normal(samples), dtype=float)
    if colorednoise is not None:
        random_state = np.random.RandomState(np.random.randint(0, 2**31 - 1))
        try:
            noise = colorednoise.powerlaw_psd_gaussian(
                beta, samples, random_state=random_state
            )
            return np.asarray(noise, dtype=float)
        except TypeError:
            pass

    # Fallback keeps the research workflow available without the optional
    # colorednoise package and remains controlled by MaskingConfig.random_seed.
    frequencies = np.fft.rfftfreq(samples, d=1.0 / sr)
    amplitudes = np.ones_like(frequencies)
    nonzero = frequencies > 0
    amplitudes[nonzero] = frequencies[nonzero] ** (-beta / 2.0)
    amplitudes[~nonzero] = 0.0
    spectrum = np.fft.rfft(np.random.standard_normal(samples)) * amplitudes
    noise = np.fft.irfft(spectrum, n=samples)
    deviation = np.std(noise)
    if deviation > 0:
        noise /= deviation
    return np.asarray(noise, dtype=float)
